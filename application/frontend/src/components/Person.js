    ...
    return (
      ...
      >
        <button
          ...
          onClick={updateLine}
          style={{
            backgroundColor: isColorFlipped ? "black" : "#EFEFEF",
            color: isColorFlipped ? color : "black",
            boxShadow: isColorFlipped ? `0 0 3px ${color}` : "none",
          }}
          disabled={(!isClickable && !isApiSuccess) || isMuted}
        >
          {data.name}
        </button>
        <img src={data.icon} alt="Icon" className="icon mr-2 spacing" />
        <div
          className="form-control spacing no-shadow"
          style={{
            opacity: data.opacity,
            transition: data.opacity === 1 ? "none" : "opacity 5s ease-in-out",
          }}
        >
          {data.currentLine}
        </div>
